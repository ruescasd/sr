...
impl Message {
    fn collides(&self, other: &Message) -> bool {
        ...
        match (self, other) {
            (Message::Shares(_, shares1, sender1), 
            Message::Shares(_, shares2, sender2)) => sender1 == sender2,
            _ => false
        }
    }
}
...
trait SetAdd<T> {
    fn add_element(&self, elem: T) -> Self;
}

impl<T: Ord + Clone> SetAdd<T> for BTreeSet<T> {
    fn add_element(&self, elem: T) -> Self {
        let mut ret = self.clone();
        // slow but we don't care yet
...
mod decide {
    ...
    ascent! {
        ...
        error(format!("duplicate message {:?}, {:?}", m1, m2)) <-- message(m1), message(m2), if m1.collides(m2);

        relation trigger_a(u32);
        trigger_a(*a) <-- message(m), if let Message::TriggerA(a) = m;
        
        relation done_a(u32);
        done_a(*a) <-- message(m), if let Message::DoneA(a) = m;

        action(Action::A(*a)) <-- trigger_a(a), !done_a(a);

        relation acc(BTreeSet<u32>);
        acc(set(1));

        acc(v.add_element(*a)) <-- acc(v), done_a(a);
    }

    pub(crate) fn program() -> super::decide::AscentProgram {
        AscentProgram::default()
    }
}
...
mod execute {
    ...
    ascent! {
        ...
        // (timestamp, trustee number)
        relation active(u32, u32);
        
        
        relation action(Action);
        relation message(Message);

        message(Message::DoneA(*a)) <-- action(a_), active(now, trustee), if let Action::A(a) = a_;
    }
...
use stateright::{Model, Property};
struct Ascent;
impl Model for Ascent {
    type State = Vec<Message>;
    type Action = Action;

    fn init_states(&self) -> Vec<Self::State> {
        // cause duplicate errors
        // let init = vec![Message::TriggerA(23), Message::TriggerA(24), Message::B(23), Message::B(45)];
        let init = vec![Message::TriggerA(23)];

        vec![init]
    }

    fn actions(&self, state: &Self::State, actions: &mut Vec<Self::Action>) {
        let mut prog = decide::program();
    ...
    fn next_state(&self, last_state: &Self::State, action: Self::Action) -> Option<Self::State> {
        ...
        /* if messages_.len() > 0 {
            println!("* next state: {:?} + {:?}", last_state, messages_);
            new_state.append(&mut messages_);
        }*/
        /* else {
            println!("* next state: appending NOP");
    ...
    fn properties(&self) -> Vec<Property<Self>> {
        vec![Property::<Self>::sometimes("solved", |_, state| {
            state.contains(&Message::DoneA(23))
        })]
    }
}
...
mod tests {
    ...
    #[test]
    fn doesa() {
        let checker = Ascent.checker().spawn_bfs().join();
        checker.assert_properties();
    }
}*/
